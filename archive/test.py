...
def get_user_preferences():
    ...
    return event_type, location_sidebar, season, mood, time
...
def prepare_sub_df_for_output(df: pd.DataFrame, top5: bool, event_type: list, location_sidebar: list, season: list, event_subtype: list, mood: list, time: list):
    ...
    :param time: list of times that the user selected
    """
    sub_df = df[df['season'].isin(season) & df['district'].isin(location_sidebar) & df['supercategory'].isin(event_type) & df['subcategory'].isin(event_subtype) & df['stimmung'].isin(mood) & df['time_of_day'].isin(time)]
    # Only select the relevant columns
    sub_df = sub_df[['name', 'description', 'location.name', 'location.location.address.street', 'supercategory', 'subcategory', 'stimmung', 'time_of_day']]
    sub_df.columns = ['Event', 'Description', 'Location', 'Address', 'Type', 'Category', 'Flair', 'Time_of_Day']
    # create a dictionary with the location as key and the address and category as values
    location_dict = {}
    for index, row in sub_df.iterrows():
        # the key of the dict consists of location and time of day
        key = f"'{row['Location']}', '{row['Time_of_Day']}'"
        if key not in location_dict:
            location_dict[key] = [row['Address'], row['Type'], row['Category'], row['Flair'], 1]
        else:
            location_dict[key][4] += 1
    # count number of events per location
    events_per_location_count_df = sub_df.groupby(['Location', 'Time_of_Day']).count()
    ...
    if top5 == True:
        ...
        for location in top_5_locations:
            ...
            if location_key in location_dict:
                star_rating = min(location_dict[location_key][4], 5)
                num_of_events = location_dict[location_key][4]
                new_entry = pd.DataFrame([[location[0], location_dict[location_key][0], location_dict[location_key][1], location_dict[location_key][2], location_dict[location_key][3], star_rating*'⭐', num_of_events]], columns=['Location', 'Address', 'Type', 'Category', 'Flair', 'Popularity', 'Number of Events'])
                top_5_locations_df = pd.concat([top_5_locations_df, new_entry], ignore_index=True)
        ...
        for location in all_locations:
            ...
            if location_key in location_dict:
                ...
                num_of_events = location_dict[location_key][4]
                new_entry = pd.DataFrame([[location[0], location_dict[location_key][0], location_dict[location_key][1], location_dict[location_key][2], location_dict[location_key][3], star_rating*'⭐', num_of_events]], columns=['Location', 'Address', 'Type', 'Category', 'Flair', 'Popularity', 'Number of Events'])
                all_locations_df = pd.concat([all_locations_df, new_entry], ignore_index=True)
...
def display_locations(df: pd.DataFrame, selected_tab: str, event_type, location_sidebar, season, event_subtype, mood, time):
    if selected_tab == "Top 5 Locations":
        st.subheader('Top 5 Locations for your preferences🚀')
        output_df = prepare_sub_df_for_output(df, top5=True, event_type=event_type, location_sidebar=location_sidebar, season=season, event_subtype=event_subtype, mood=mood, time=time)
        ...
        output_df = prepare_sub_df_for_output(df, top5=False, event_type=event_type, location_sidebar=location_sidebar, season=season, event_subtype=event_subtype, mood=mood, time=time)
        st.write(output_df.to_html(escape=False, index=False, justify='center'), unsafe_allow_html=True)
...
def main():
    ...
    display_title()
    event_type, location_sidebar, season, mood, time = get_user_preferences()
    ...
    if selected_tab == "Information about subset of dataset":
        ...
        display_locations(df, selected_tab, event_type, location_sidebar, season, event_subtype, mood, time)
        show_no_of_events_used(df)
        #generate_latitude_longitude_chart(df)
